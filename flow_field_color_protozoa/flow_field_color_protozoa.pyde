...
perlin_randomness_scale = 1
uniform_randomness_scale = 10
top_neighbor_scale = 0.1   # MUST be less than 1
    ...
    for i in range(rows):
        if i == 0:
            ...
            for j in range(cols):
                ...
                '''
                new_row.append(field[i-1][j]*top_neighbor_scale + (noise(i * cell_size) + noise(j * cell_size) -1) * perlin_randomness_scale + random(10) * uniform_randomness_scale )
    ...
    for i in range(int(sqrt(num_lines))):
        ...
        for j in range(int(sqrt(num_lines))):
            ...
            for n in range(num_steps):
                stroke(color(j * 255 / int(sqrt(num_lines)),200,200,100))
                #stroke(line_color)
                curveVertex(x, y)
